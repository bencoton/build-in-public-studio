...
- **This bites every jsonb array column, not just the one that crashed.** `moments.source_ref` (also jsonb) returned a string too. Its `Array.isArray(source_ref)` guard did NOT parse — it silently fell through to `[]`, so `source_refs` was empty on every DB *read* (the in-memory generation path is unaffected; the damage was to re-reads: moment-card badges, regenerate context, on-demand Reddit source material). **An `Array.isArray`-only guard hides the bug instead of fixing it.** ✅ **RESOLVED 2026-06-21** — coerced at all three read sites (`getMomentById`, `getMomentsByGeneration`, and the `claude-regenerate` mapper of `getDraftWithMoment`'s row); ground-truth probe confirmed `source_refs` and the on-demand `relevantCommits` filter went from 0 → populated.
- **Fix recipe:** Coerce at the **read choke point** (the data-access mapper), so every consumer downstream gets a real array — don't guard at each render site. The canonical helper is **`coerceStringArray` in `src/lib/json.ts`** (`if typeof === "string" → JSON.parse (catch → null); then if !Array.isArray → null; else filter to the element type`) — used by both `subreddits.prepost_checklist` and `moments.source_ref`. Apply it wherever the row is shaped (every SELECT/RETURNING mapper); don't re-implement it. Keep a defensive `Array.isArray(x) ? x : []` at any `.join`/`.map` render site as belt-and-braces, but the real fix is the boundary.
