...
- **2026-06-21** — **Restored moment source-ref context (badges + draft quality).** A jsonb column (`moments.source_ref`) was coming back from the database as a string and silently reading as empty, so moment cards showed no commit SHAs and on-demand Reddit / regenerate drafts had lost their commit/note context. Now coerced to a real array at the read boundary via a shared helper (same root cause as the Settings build fix; BIPS-L7). No new dependency.
- **2026-06-21** — **Fixed a build-blocking crash on Settings + made DB pages dynamic.** The production build was failing while prerendering `/settings` because a jsonb checklist column comes back from postgres.js as a JSON string, not an array. Now coerced to an array at the data-access boundary, and all DB-backed pages render per-request (`force-dynamic`) instead of being statically prerendered against the database at build time. Captured as lesson BIPS-L7. No new dependency.
- **2026-06-21** — **Subreddits are now user-managed (no code change to add one).** Settings → Manage subreddits: add (name only; tone/self-promo/checklist/flair optional), edit, remove. The four curated subs are seeded on first run with their original rules. The per-project selector and per-moment picker list the catalog; a sub with no rules still drafts (generic journey steer) and hides its checklist block. Removing a sub never breaks historical drafts that used it. Backed by a new `subreddits` table (migration 0005); the old fixed-slug `CHECK` is gone. No new dependency.
- **2026-06-21** — **Per-project Reddit subs + auto-generation on the dashboard.** Settings now has a "Reddit auto-generation" section: pick the target subreddits once per project (up to 3; empty = off, opt-in). When you click Generate, after a repo's moments + X/IH drafts are saved, a separate bounded Reddit pass auto-drafts one tailored post per (new moment × selected sub) for that repo — no per-moment picking. The per-repo progress row shows a "drafting Reddit…" sub-step and a "+N Reddit" / failure note; a Reddit failure doesn't block the queue. The per-moment Reddit tab remains as an override. Stored as `reddit.subs.<repo>` in settings (no schema change). Cron path intentionally left for a follow-up (its 60s budget needs addressing first). No new dependency.
- **2026-06-21** — **Per-repo "Generate" can no longer hang forever.** The per-repo generate path now time-boxes its GitHub sync (10s, non-fatal — falls back to cached commits) and its generation (90s), every Anthropic call has a 60s ceiling, and a stalled/failed repo surfaces a clear timeout error and lets the queue continue to the next repo instead of spinning indefinitely. Per-repo stage timings (sync / identify / draft) are logged so a future stall shows where. No new dependency.
