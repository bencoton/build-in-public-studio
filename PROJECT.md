...
**Phase 1b — Tech stack migration.** Stages 9b.1 + 9b.2 complete: Supabase project provisioned, schema applied, every `src/lib/` DB helper ported to `@supabase/supabase-js`, `db.ts` deleted, no more `node:sqlite` references anywhere. App runs end-to-end against Postgres — verified `/notes`, `/settings`, dashboard generation, per-variant lifecycle, history filters, ratings. Voice-learning loop deferred to user acceptance testing. **Stage 9b.3 next: deploy to Vercel** for a live URL + dev-server-independence. Then 9b.4 wires the Monday-9am Vercel Cron job.
