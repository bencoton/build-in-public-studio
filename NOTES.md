...
## 2026-08-02 — Database TLS hardened: `sslmode=verify-full` on both Neon endpoints

Applied the `verify-full` fix proven in `duckworth-reckons`. `require` does not mean "verified TLS" — it means encrypted against a certificate nobody checked. The standing rule now lives in `docs/Tech-Stack.md` Part 6 (that file is gitignored here; the canonical copy is in the Ways of Working folder and needs the same paste).

- **Doppler `dev` and `prd`** — `DATABASE_URL` rewritten from `sslmode=require` → `sslmode=verify-full`. Written over **STDIN** (an argv value can silently no-op on reserved characters), substitution **anchored on `sslmode=`** at a `?`/`&` boundary so the adjacent `channel_binding=require` couldn't be eaten. Guards before each write: still a `postgresql://` URL, carries both `verify-full` and an unchanged `channel_binding`, exactly one `sslmode` param, pre-query portion byte-identical, and whole-URL length delta equal to the sslmode token delta alone (+4 chars, as expected). Re-read from the API after writing rather than trusting the exit code — stored value byte-identical to intended on both. `channel_binding=require` intact on both.
- **The URL change alone was inert — the real bug was in code.** `src/lib/db.ts` passed `ssl: "require"` explicitly. Two findings from reading the installed `postgres.js`: (1) `ssl: 'require'|'allow'|'prefer'` sets **`rejectUnauthorized: false`** (`connection.js:284`) — certificate verification was off *today*, not at some future v9; (2) `parseOptions` resolves `k in o ? o[k] : query[k]` (`index.js:474`), so the **explicit option overrides `sslmode=` in the URL**. Changed to `ssl: "verify-full"`, which isn't special-cased and so falls through to Node's `tls.connect` defaults (`rejectUnauthorized: true`) with `servername` set. Comment added at the call site explaining why it must never go back to `require`. Only connection site in the repo (grep for `postgres(` returns one hit).
- **Proof, both endpoints:** real query (`select current_database(), version()` → `neondb | PostgreSQL 17.10`) **plus a negative control** — the same endpoint with an empty CA store must be rejected, and was, with `UNABLE_TO_GET_ISSUER_CERT_LOCALLY`. Without that control a passing connection proves nothing. Dev proven before prd was touched.
- **Deploy:** `vercel deploy --prod` → **`dpl_J4qdpKMcaCcsHLUyB7S4LzyuURJ8`** (READY). Alias resolved independently via `vercel inspect https://build-in-public-studio.vercel.app` → confirmed pointing at that same id, not merely trusting the CLI's "Aliased" line. Smoke test: `/` 200, `/settings` 200 and rendering real DB-backed content (subreddit/banned-word/schedule strings present), so the pooled Neon read path works under `verify-full` in production.
- **Gates:** `npx tsc --noEmit` clean, `npm run lint` clean.
- **Left alone deliberately — needs Ben's eyes.** Vercel holds its **own** copy of `DATABASE_URL` (created 66d ago; unchanged after the Doppler write, so a live Doppler→Vercel sync is unconfirmed). Its `sslmode` may still read `require`. Production is safe regardless *because the code option overrides the URL*, but the strings are now potentially out of agreement. I did **not** overwrite it: reading the decrypted value was blocked by the permission classifier, and overwriting blind risked repointing production at a different endpoint if the two ever diverged. **Action for Ben:** open the Vercel dashboard, compare that value against Doppler `prd`, and if the endpoint matches, set `sslmode=verify-full` there too, then redeploy.
...
- *`humanize()` live run (DONE 2026-06-25, Ben's key, via the `ts-resolve-hook` run command):* **PASS BAR true.** `x_thread` 14 → 1 tells; `ih_long` 20 → 0 tells. `[VERIFY]` preserved 1/1 on both; **no numbers dropped**. Token usage ~2.7k in / ~0.5k out per draft. The audit pass showed judgment, not mechanical stripping — it deliberately *kept* a natural conversational em-dash in the X thread rather than removing it as a tell. Appendix B closed; remaining optional work is expanding the sample toward 20 drafts for a tighter median. **Phase 1 (P0) complete.**
